...
class UserApiKafkaResourceIT {
    ...
    void setup() {
        ...
        UserApiKafkaResource kafkaResource = new UserApiKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/user-api-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc.perform(get("/api/user-api-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
